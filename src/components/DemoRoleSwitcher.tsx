...
import { User, ChevronDown, Check, Info, LogOut, ShieldCheck, Key } from 'lucide-react';
...
export default function DemoRoleSwitcher() {
  ...
  if (!mounted) {
    return (
      <div className="h-9 px-3 bg-slate-900 border border-slate-800 rounded-lg flex items-center justify-center animate-pulse">
        <span className="text-[10px] text-slate-500 font-mono">Memuat...</span>
      </div>
    );
  }

  // If Supabase login is active
  if (isSupabaseConfigured && !isDemoMode && user && profile) {
    const roleLabel = profile.role === 'admin' ? 'Pemilik UMKM' : 'Kasir';
    const badgeColor = profile.role === 'admin' 
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          type="button"
          className={`flex items-center gap-2 px-3 py-1.5 bg-slate-950 border rounded-xl hover:border-slate-700 transition-all font-mono text-[10px] text-left leading-tight ${badgeColor}`}
        >
          <ShieldCheck className="w-3.5 h-3.5" />
          <div className="flex flex-col">
            <span className="font-bold">{roleLabel}</span>
            <span className="text-[8px] opacity-75 font-sans">{profile.full_name}</span>
          </div>
        ...
        {isOpen && (
          <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 p-2 animate-slide-in">
            <div className="px-3 py-2 border-b border-slate-850 flex items-start gap-1.5 mb-1.5 bg-emerald-950/20 rounded-t-lg">
              <Key className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0 mt-0.5" />
              <div className="flex flex-col">
                <span className="text-[9px] font-bold text-white font-mono uppercase">Login Supabase Aktif</span>
                <p className="text-[8px] text-slate-400 leading-normal font-sans">
                  Anda masuk menggunakan data Supabase. Mode demo dinonaktifkan.
                </p>
              </div>
            </div>

            <div className="px-3 py-1.5 flex flex-col gap-0.5 text-left border-b border-slate-850 mb-1.5">
              <span className="text-[9px] text-slate-500 font-mono uppercase">Info Akun</span>
              <span className="text-[10px] text-white font-bold">{profile.full_name}</span>
              <span className="text-[9px] text-slate-400 font-sans">{profile.email}</span>
            </div>
            ...
            <button
              ...
              type="button"
              className="w-full text-left p-2 rounded-lg flex items-center gap-2 transition-all hover:bg-red-500/10 text-red-400"
            >
              <LogOut className="w-3.5 h-3.5" />
              <span className="text-[10px] font-bold">Keluar Akun</span>
            </button>
          </div>
        )}
      </div>
  ...
  // Otherwise, render default demo mode role switcher
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="flex flex-col">
          <span className="font-bold">{currentConfig.label} (Demo)</span>
        ...
        <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 p-2 animate-slide-in">
          <div className="px-3 py-2 border-b border-slate-850 flex items-start gap-1.5 mb-1.5">
            ...
            <p className="text-[9px] text-slate-400 leading-normal font-sans">
              Mode demo aktif. Mengganti role akan mensimulasikan otorisasi UI tanpa menghapus data.
            </p>
          </div>
